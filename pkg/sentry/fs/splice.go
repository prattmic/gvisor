...
func Splice(ctx context.Context, dst *File, src *File, opts SpliceOpts) (int64, error) {
	...
	// Second, it prevents Splice deadlocks. Only stream-oriented files
	// implement the ReadFrom and WriteTo methods directly, and since such
	// stream-oriented files are referred to by a unique fs.File object, we
	// know that the file mutex takes strict precedence over internal
	// locks. Since we enforce lock ordering here, we can't deadlock by
	// using a stream in two different splice operations simultaneously.
	...
	// the source and destination are not pipes. This is because if
	// we block at some point, we could lose data. If the source is
	// not a pipe then reading is not destructive; if the destination
	// is not a pipe then it is guaranteed not to block.
