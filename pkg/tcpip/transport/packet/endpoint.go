...
// Package packet provides the implementation of packet sockets (see packet(7)). Packet
...
func newEndpoint(stack *stack.Stack, cooked bool, netProto tcpip.NetworkProtocolNumber, waiterQueue *waiter.Queue) (tcpip.Endpoint, *tcpip.Error) {
	ep := &endpoint{
		stack:         stack,
		cooked:        cooked,
		netProto:      netProto,
	...
	if err := stack.RegisterPacketEndpoint(0, netProto, ep); err != nil {
		return nil, err
	}
	return ep, nil
}
...
func (ep *endpoint) Write(payload tcpip.Payload, opts tcpip.WriteOptions) (int64, <-chan struct{}, *tcpip.Error) {
	...
	// opts.To has has destination information.
	if len(opts.To.Addr) != header.EthernetAddressSize {
		return 0, nil, tcpip.ErrBadAddress
	}
	...
	payloadVV := buffer.View(payloadBytes).ToVectorisedView()

	if ep.cooked {
		// "Packets sent through a SOCK_DGRAM packet socket get a
		// suitable physical-layer header based on the information in
		// the sockaddr_ll destination address before they are queued."
		// - packet(7)
		err = ep.stack.WritePacket(opts.To.NIC, tcpip.LinkAddress(opts.To.Addr), opts.To.Protocol, payloadVV)
	} else {
		// "SOCK_RAW  packets are passed to and from the device driver
		// without any changes in the packet data... When transmitting
		// a packet, the user-supplied buffer should contain the
		// physical-layer header.  That packet is then queued
		// unmodified to the network driver of the interface defined by
		// the destination address." - packet(7)
		err = ep.stack.WriteRawPacket(opts.To.NIC, payloadVV, opts.To.Protocol)
	}

	return int64(len(payloadBytes)), nil, err
}
...
// Shutdown implements tcpip.Endpoint.Shutdown. Packet sockets cannot be used with
// Shutdown, and this function always returns tcpip.ErrNotSupported.
...
func (ep *endpoint) Bind(addr tcpip.FullAddress) *tcpip.Error {
	// TODO(gvisor.dev/173): Add Bind support.
...
// SetSockOpt implements tcpip.Endpoint.SetSockOpt. Packet sockets cannot be used with
// SetSockOpt, and this function always returns tcpip.ErrNotSupported.
func (ep *endpoint) SetSockOpt(opt interface{}) *tcpip.Error {
	return tcpip.ErrNotSupported
}

// GetSockOpt implements tcpipInt.Endpoint.GetSockOpt.
